...
async def upload_files(output_folder, chunk_id, xmin, ymin):
    ...
    async def upload(file):
        file_name = os.path.basename(file).replace('.laz_depr.png', '')
        x, y = map(int, file_name.split('_')[1:3])
        
        if xmin <= x < xmin + 5000 and ymin <= y < ymin + 5000:
            x_fix, y_fix = x // 200, (6553600 - y) // 200
            return asyncio.to_thread(s3_nz.upload_file, file, 'nzomap', f'tiles/15/{x_fix}/{y_fix}.png')
...
def download_file(s3_client, bucket, file_key, destination):
    """Wrapper to download a file using S3 client."""
    s3_client.download_file(bucket, file_key, destination)

async def download_files_concurrently(s3_client, files, download_dir):
    """Downloads all files in parallel using a thread pool."""
    ...
    with ThreadPoolExecutor() as executor:
        tasks = [
            loop.run_in_executor(
                executor,
                download_file,
                s3_client, 'pc-bulk', file.replace('https://opentopography.s3.sdsc.edu/pc-bulk/', ''),
...
async def process_chunk(download_semaphore, pullauta_semaphore,i):
    ...
    async with download_semaphore:
        ...
        await download_files_concurrently(s3, file_list, downloaded_files_dir)

        print(f"Finished downloading data for chunk {area_uuid}")
    ...
    async with pullauta_semaphore:
        ...
        try:
            ...
            print(f"Failed to download osm.zip for chunk {area_uuid}")
            pass
    ...
    try:
        shutil.rmtree(process_dir)
        print(f"Cleaned up temporary files for chunk {area_uuid}")

    except:
        print(f"Warning failed to clean up temporary files for chunk {area_uuid}")
        pass
